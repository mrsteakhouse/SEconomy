...
public abstract class DBUtil
{
	 ...
	 */
	public DBUtil(JavaPlugin javaPlugin)
	{
		...
		try
		{
			// Find the "getClassLoader" method and make it "public" instead of
			// "protected"
			Method method = JavaPlugin.class
	 ...
	 */
	public void initializeDatabase(String driver, String url, String username,
			String password, String isolation) throws Exception
	{
	...
	private void prepareDatabase(String driver, String url, String username,
			String password, String isolation)
	{
	...
	private void loadDatabase()
	{
		...
		try
		{
			...
			// URLConnection
			cacheField = URLConnection.class
					.getDeclaredField("defaultUseCaches");
		...
		} finally
		{
			...
			{
				System.out
						.println("Failed to revert the \"defaultUseCaches\"-field back to its original value, URLConnection-caching remains disabled.");
			}
		}
	}
	...
	{
		input = input.replaceAll("\\{DIR\\}", javaPlugin.getDataFolder()
				.getPath().replaceAll("\\\\", "/") + '/');
		input = input.replaceAll("\\{NAME\\}", javaPlugin.getDescription()
	...
	private static String validateCreateDDLSqlite(String oldScript)
	{
		try
		{
			...
			while ((currentLine = scriptReader.readLine()) != null)
			{
				...
				if (currentLine.startsWith("create table"))
				{
					...
					foundTables.put(currentLine.split(" ", 3)[2],
							scriptLines.size() - 1);
				} else if (!currentLine.isEmpty()
						&& currentLine.charAt(0) == ';' && currentTable != null
						&& !currentTable.isEmpty())
				{
					...
					previousLine = previousLine.substring(0,
				...
				} else if (currentLine.startsWith("alter table"))
				{
					...
					if (alterTableLine[3].startsWith("add constraint"))
					{
						...
						// CONSTRAINT
						String[] addConstraintLine = alterTableLine[3].split(
								" ", 4);
						...
						if (addConstraintLine[3].startsWith("foreign key"))
						{
							...
							// table
							int tableLastLine = foundTables
									.get(alterTableLine[2]) + tableOffset;

							// Add a "," to the previous line
							scriptLines.set(tableLastLine - 1,
									scriptLines.get(tableLastLine - 1) + ',');

							// Add the constraint as a new line - Remove the ";"
							// on the end
							String constraintLine = String.format("%s %s %s",
									addConstraintLine[1], addConstraintLine[2],
									addConstraintLine[3]);
							scriptLines.add(
									tableLastLine,
									constraintLine.substring(0,
											constraintLine.length() - 1));
		...
		} catch (Exception ex)
		{
			...
			// wrong
			throw new RuntimeException(
					"Failed to validate the CreateDDL-script for SQLite", ex);
		}
	}
	...
	protected void prepareDatabaseAdditionalConfig(
			DataSourceConfig dataSourceConfig, ServerConfig serverConfig)
	{
	}
